#!/usr/bin/env python3
"""
Pytest tests for content keyword analyzer - standalone version.
"""
...
class ContentAnalyzerCore:
    """Core content analysis functionality for testing."""
...
class TestContentKeywordAnalyzer:
    ...
    def setup_method(self):
        ...
        self.analyzer = ContentAnalyzerCore()
        self.sample_content = """
    ...
    def test_keyword_density_accuracy(self):
        """Test keyword density calculation accuracy."""
        # Simple test content: 10 words total
        content = "gitlab android app gitlab android mobile gitlab android development team"

        keyword_density = self.analyzer.calculate_keyword_density(content)

        # gitlab appears 3 times in 10 words = 30%
        assert abs(keyword_density["gitlab"] - 30.0) < 0.1

        # android appears 3 times in 10 words = 30%
        assert abs(keyword_density["android"] - 30.0) < 0.1

        # "gitlab android" phrase appears 3 times = 30%
        assert abs(keyword_density["gitlab android"] - 30.0) < 0.1

    def test_semantic_coverage_accuracy(self):
        """Test semantic coverage counting accuracy."""
        content = "android mobile apps for android development and mobile teams"

        semantic_groups = {
            "mobile_tech": ["android", "mobile"]
        }

        coverage = self.analyzer.analyze_semantic_coverage(content, semantic_groups)

        # Should find: android (2) + mobile (2) + mobiles (variation) = at least 4
        assert coverage["mobile_tech"] >= 4

    def test_case_insensitive_matching(self):
        """Test that keyword matching is case insensitive."""
        content = "GitLab Android MOBILE App Development"
