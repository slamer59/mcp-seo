...
class TestPageRankToolsIntegration:
    ...
    async def test_analyze_pagerank_full_workflow(self, mock_kuzu_class, mock_builder_class):
        ...
        mock_builder.build_link_graph_from_sitemap = AsyncMock(return_value={
            'total_pages': 10,
            'total_links': 25,
            'crawled_pages': 10,
            'discovered_links': 25
        ...
        with patch('mcp_seo.tools.graph.pagerank_tools.PageRankAnalyzer') as mock_analyzer_class:
    ...
    @patch('mcp_seo.tools.graph.pagerank_tools.LinkGraphBuilder')
    @patch('mcp_seo.tools.graph.pagerank_tools.KuzuManager')
    async def test_analyze_pagerank_with_urls(self, mock_kuzu_class, mock_builder_class):
        ...
        mock_builder = MagicMock()
        mock_builder.build_link_graph_from_urls = AsyncMock(return_value={
            'total_pages': 5,
            'total_links': 10
        })
        mock_builder_class.return_value = mock_builder
        ...
        with patch('mcp_seo.tools.graph.pagerank_tools.PageRankAnalyzer') as mock_analyzer_class:
            mock_analyzer = MagicMock()
            mock_analyzer.calculate_pagerank.return_value = {'https://example.com/': 0.5}
            mock_analyzer.generate_analysis_summary.return_value = {
            ...
            mock_builder.build_link_graph_from_urls.assert_called_once_with(['https://example.com'])
            assert result['parameters']['used_sitemap'] is False

    @patch('mcp_seo.tools.graph.pagerank_tools.LinkGraphBuilder')
    @patch('mcp_seo.tools.graph.pagerank_tools.KuzuManager')
    async def test_analyze_pagerank_error_handling(self, mock_kuzu_class, mock_builder_class):
    ...
    async def test_find_orphaned_pages_tool(self, mock_kuzu_class):
        ...
        mock_kuzu.get_page_data.return_value = [
            ...
            {'url': 'https://example.com/blog/post', 'in_degree': 0, 'path': '/blog/post'},
        ]
        mock_kuzu_class.return_value = mock_kuzu
    ...
    @patch('mcp_seo.tools.graph.pagerank_tools.KuzuManager')
    async def test_optimize_internal_links_tool(self, mock_kuzu_class, mock_analyzer_class, mock_builder_class):
        ...
        mock_builder.get_link_opportunities.return_value = {
            ...
            'low_outlink_pages': [
                {'url': 'https://example.com/low', 'out_degree': 1}
            ],
    ...
    async def test_pydantic_models_validation(self):
        ...
        assert valid_request.domain == "https://example.com"
        ...
        assert orphan_request.domain == "https://example.com"
    ...
    async def test_error_logging(self, mock_logger):
        ...
        with patch('mcp_seo.tools.graph.pagerank_tools.KuzuManager', side_effect=Exception("Test error")):
            result = await analyze_pagerank(request)
    ...
    async def test_concurrent_tool_execution(self):
        ...
        requests = [
            PageRankRequest(domain=f"https://example{i}.com")
            for i in range(3)
        ]
        ...
        with patch('mcp_seo.tools.graph.pagerank_tools.KuzuManager') as mock_kuzu_class:
            ...
            with patch('mcp_seo.tools.graph.pagerank_tools.LinkGraphBuilder') as mock_builder_class:
                mock_builder = MagicMock()
                mock_builder.build_link_graph_from_sitemap = AsyncMock(return_value={'error': 'No pages'})
                mock_builder_class.return_value = mock_builder
