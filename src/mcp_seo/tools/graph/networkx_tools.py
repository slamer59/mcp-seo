...
class NetworkXAnalysisRequest(BaseModel):
    ...
    domain: str = Field(description="Domain to analyze (must have existing link graph)")

    @field_validator("domain", mode="before")
    @classmethod
    def validate_domain_url(cls, v):
        """Validate and normalize domain URL, handling localhost cases."""
        if not v:
...
class CentralityAnalysisRequest(BaseModel):
    ...
    top_k: int = Field(
        default=20, ge=1, le=100, description="Number of top pages to return per metric"
    )
...
class CommunityDetectionRequest(BaseModel):
    ...
    domain: str = Field(description="Domain to analyze")
    algorithm: str = Field(
        default="louvain", description="Algorithm to use (louvain, greedy_modularity)"
    )
    min_community_size: int = Field(
        default=3, ge=1, description="Minimum community size to report"
    )
...
class ConnectorAnalysisRequest(BaseModel):
    ...
    domain: str = Field(description="Domain to analyze")
    min_betweenness: float = Field(
        default=0.0, ge=0.0, description="Minimum betweenness centrality threshold"
    )
...
def register_networkx_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    async def analyze_centrality(params) -> Dict:
        """
        ...
        try:
            ...
            request = CentralityAnalysisRequest.model_validate(params)
            ...
            with KuzuManager() as kuzu_manager:
                ...
                if not pages_data:
                    return {
                        "error": "No link graph data found. Run 'build_link_graph' or 'analyze_pagerank' first."
                    }
                ...
                for metric in request.metrics:
                    ...
                    if (
                        metric_key in list(centrality_data.values())[0]
                ...
                recommendations = _generate_centrality_recommendations(
                    top_pages, centrality_data
                )
    ...
    @mcp.tool()
    async def detect_communities(params) -> Dict:
        """
        ...
        try:
            ...
            if isinstance(params, str):
                params = json.loads(params)
            request = CommunityDetectionRequest.model_validate(params)
            ...
            with KuzuManager() as kuzu_manager:
                ...
                networkx_analyzer = NetworkXAnalyzer(kuzu_manager)
                community_results = networkx_analyzer.detect_communities(
                    request.algorithm
                )
                ...
                content_recommendations = _generate_community_recommendations(
                    filtered_communities, community_results["modularity_score"]
                )
    ...
    @mcp.tool()
    async def analyze_site_structure(params) -> Dict:
        """
        ...
        try:
            ...
            if isinstance(params, str):
                params = json.loads(params)
            request = NetworkXAnalysisRequest.model_validate(params)
            ...
            with KuzuManager() as kuzu_manager:
                ...
                architecture_recommendations = _generate_structure_recommendations(
                    structural_results
                )
    ...
    @mcp.tool()
    async def analyze_navigation_paths(params) -> Dict:
        """
        ...
        try:
            ...
            with KuzuManager() as kuzu_manager:
                ...
                navigation_recommendations = _generate_navigation_recommendations(
                    path_results
                )
                ...
                result = {
                    ...
                    "connectivity_analysis": {
                        "strongly_connected_components": path_results[
                    ...
                    "navigation_issues": {
                        ...
                        "easy_to_reach_pages": path_results["easy_to_reach_pages"][
                            :10
    ...
    @mcp.tool()
    async def find_connector_pages(params) -> Dict:
        """
        ...
        try:
            ...
            if isinstance(params, str):
                params = json.loads(params)
            request = ConnectorAnalysisRequest.model_validate(params)
            ...
            with KuzuManager() as kuzu_manager:
                ...
                filtered_connectors = [
                    page
                    for page in connector_results["connector_pages"]
    ...
    @mcp.tool()
    async def comprehensive_graph_analysis(params) -> Dict:
        """
...
def _generate_centrality_recommendations(
    top_pages: Dict, centrality_data: Dict
) -> List[Dict]:
    ...
    for url, data in centrality_data.items():
        if (
            data["eigenvector_centrality"] > 0.01
            and data["betweenness_centrality"] < 0.01
        ):
...
def _generate_community_recommendations(
    communities: List[Dict], modularity: float
) -> List[Dict]:
    ...
    small_communities = [
        c for c in communities if c["size"] <= 3 and c["external_edges"] == 0
    ]
...
def _generate_linking_strategy(
    connectors: List[Dict], opportunities: List[Dict], cross_sections: Dict
) -> List[Dict]:
    ...
    isolated_sections = [
        section
        for section, connections in cross_sections.items()
...
def _generate_analysis_summary(results: Dict) -> Dict:
    ...
    # Extract key metrics
    if "centrality" in results and "error" not in results["centrality"]:
        summary["key_metrics"]["total_pages"] = results["centrality"].get(
            "total_pages", 0
        )

    if "communities" in results and "error" not in results["communities"]:
        summary["key_metrics"]["modularity_score"] = results["communities"].get(
            "modularity_score", 0
        )
        summary["key_metrics"]["num_communities"] = results["communities"].get(
            "num_communities", 0
        )

    if "paths" in results and "error" not in results["paths"]:
        path_metrics = results["paths"].get("path_metrics", {})
        summary["key_metrics"]["avg_path_length"] = path_metrics.get(
            "average_path_length", 0
        )
        summary["key_metrics"]["connectivity_ratio"] = results["paths"].get(
            "connectivity_ratio", 0
        )

    if "structure" in results and "error" not in results["structure"]:
        summary["key_metrics"]["max_core_number"] = results["structure"][
