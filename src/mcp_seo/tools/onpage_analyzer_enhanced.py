...
class EnhancedOnPageAnalyzer:
    """Enhanced OnPage SEO analysis tool with advanced reporting and recommendations."""
    ...
    def get_analysis_summary(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "optimization_priorities": self._generate_optimization_priorities(summary_data, recommendations)
            }
        ...
        try:
            ...
            for page in pages:
                ...
                if page_analysis.get('word_count', 0) > 100:
                    content_suggestions = self._analyze_page_content_quality(page_analysis)
            ...
            result = {
                ...
                "content_optimization_summary": self._generate_content_optimization_summary(analyzed_pages),
                "page_performance_insights": self._generate_page_performance_insights(analyzed_pages)
            }
    ...
    def get_duplicate_content_analysis(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            duplicate_recommendations = self._generate_duplicate_content_recommendations(duplicate_analysis)

            result = {
                "task_id": task_id,
                "duplicate_analysis": duplicate_analysis,
                "duplicate_content_recommendations": duplicate_recommendations,
                "prioritized_fixes": self._prioritize_duplicate_content_fixes(duplicate_analysis)
            }
    ...
    def get_lighthouse_analysis(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_recommendations = self._generate_performance_recommendations(lighthouse_analysis)

            result = {
                "task_id": task_id,
                "lighthouse_analysis": lighthouse_analysis,
                "performance_recommendations": performance_recommendations,
                "core_web_vitals_assessment": self._assess_core_web_vitals(lighthouse_analysis),
                "performance_optimization_plan": self._create_performance_optimization_plan(lighthouse_analysis)
            }
    ...
    def generate_comprehensive_seo_audit(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Generate comprehensive recommendations
            comprehensive_recommendations = self.recommendation_engine.generate_comprehensive_recommendations(
            ...
            audit_result = {
                ...
                "formatted_audit_report": self.reporter.generate_comprehensive_seo_report(comprehensive_recommendations),
                "action_plan": comprehensive_recommendations.get('action_plan', {}),
                "estimated_impact": self._estimate_optimization_impact(comprehensive_recommendations)
            }
    ...
    def _calculate_seo_health_score(self, summary_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        technical_deduction = min(critical_issues * 15 + high_issues * 8 + medium_issues * 3, 50)
        score -= technical_deduction
        factors['technical_issues'] = {'score': 100 - technical_deduction, 'impact': technical_deduction}
        ...
        duplicate_ratio = (duplicate_titles + duplicate_descriptions) / (crawled_pages * 2) if crawled_pages > 0 else 0
        duplicate_deduction = min(duplicate_ratio * 30, 20)
        score -= duplicate_deduction
        factors['duplicate_content'] = {'score': 100 - duplicate_deduction, 'impact': duplicate_deduction}
        ...
        factors['broken_pages'] = {'score': 100 - broken_deduction, 'impact': broken_deduction}
    ...
    def _analyze_page_content_quality(self, page_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not title:
            ...
            suggestions['title_optimization'].append(f"Expand title (currently {len(title)} chars, aim for 30-60)")
        elif len(title) > 60:
            suggestions['title_optimization'].append(f"Shorten title (currently {len(title)} chars, aim for 30-60)")

        # Meta description analysis
        meta_desc = page_data.get('meta_description', '')
        if not meta_desc:
            suggestions['meta_description_optimization'].append("Add a meta description")
        elif len(meta_desc) < 120:
            suggestions['meta_description_optimization'].append(f"Expand meta description (currently {len(meta_desc)} chars)")
        elif len(meta_desc) > 160:
            suggestions['meta_description_optimization'].append(f"Shorten meta description (currently {len(meta_desc)} chars)")

        # Content structure
        word_count = page_data.get('word_count', 0)
        if word_count < 300:
            suggestions['content_structure'].append(f"Increase content length (currently {word_count} words)")

        images_without_alt = page_data.get('images_without_alt', 0)
        if images_without_alt > 0:
            suggestions['content_structure'].append(f"Add alt text to {images_without_alt} images")

        # Performance
        load_time = page_data.get('load_time', 0)
        if load_time > 3000:
            suggestions['performance'].append(f"Improve page load time (currently {load_time/1000:.1f}s)")

        return suggestions

    def _generate_content_optimization_summary(self, analyzed_pages: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Generate summary of content optimization opportunities."""
        total_pages = len(analyzed_pages)

        thin_content_pages = len([p for p in analyzed_pages if p.get('word_count', 0) < 300])
        missing_titles = len([p for p in analyzed_pages if not p.get('title')])
        missing_descriptions = len([p for p in analyzed_pages if not p.get('meta_description')])
        ...
        return {
            ...
            'optimization_opportunities': {
                'thin_content_pages': {
                    ...
                    'percentage': (thin_content_pages / total_pages * 100) if total_pages > 0 else 0,
                    'priority': 'Medium'
                },
                'missing_titles': {
                    'count': missing_titles,
                    'percentage': (missing_titles / total_pages * 100) if total_pages > 0 else 0,
                    'priority': 'High'
                },
                'missing_meta_descriptions': {
                    'count': missing_descriptions,
                    'percentage': (missing_descriptions / total_pages * 100) if total_pages > 0 else 0,
                    'priority': 'Medium'
                },
                'slow_loading_pages': {
                    'count': slow_pages,
                    'percentage': (slow_pages / total_pages * 100) if total_pages > 0 else 0,
                    'priority': 'High'
                }
            },
            'recommendations': [
    ...
    def _generate_page_performance_insights(self, analyzed_pages: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Generate performance insights for analyzed pages."""
        if not analyzed_pages:
            return {}

        load_times = [p.get('load_time', 0) for p in analyzed_pages if p.get('load_time', 0) > 0]
        page_sizes = [p.get('page_size', 0) for p in analyzed_pages if p.get('page_size', 0) > 0]
        ...
        insights = {
            'performance_metrics': {
                ...
                'average_page_size': sum(page_sizes) / len(page_sizes) if page_sizes else 0,
        ...
        if avg_load_time > 3000:
            insights['performance_recommendations'].append("Overall site speed needs improvement - average load time is over 3 seconds")

        slow_pages_count = insights['performance_metrics']['pages_over_3_seconds']
        if slow_pages_count > 0:
            insights['performance_recommendations'].append(f"{slow_pages_count} pages load slower than 3 seconds and need optimization")

        return insights

    def _generate_duplicate_content_recommendations(self, duplicate_analysis: Dict[str, Any]) -> List[Dict[str, str]]:
        """Generate specific recommendations for duplicate content issues."""
        recommendations = []

        duplicate_titles_count = duplicate_analysis.get('duplicate_title_tags', {}).get('count', 0)
        ...
        duplicate_descriptions_count = duplicate_analysis.get('duplicate_meta_descriptions', {}).get('count', 0)
        ...
        duplicate_h1_count = duplicate_analysis.get('duplicate_h1_tags', {}).get('count', 0)
    ...
    def _prioritize_duplicate_content_fixes(self, duplicate_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        desc_count = duplicate_analysis.get('duplicate_meta_descriptions', {}).get('count', 0)
        if desc_count > 0:
    ...
    def _generate_performance_recommendations(self, lighthouse_analysis: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    def _assess_core_web_vitals(self, lighthouse_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lcp = lighthouse_analysis.get('largest_contentful_paint', {}).get('value', 0)
        if lcp <= 2500:
            assessment['metrics']['lcp'] = {'status': 'good', 'value': lcp, 'threshold': '≤2.5s'}
        elif lcp <= 4000:
            assessment['metrics']['lcp'] = {'status': 'needs_improvement', 'value': lcp, 'threshold': '≤2.5s'}
            assessment['overall_status'] = 'needs_improvement'
        else:
            assessment['metrics']['lcp'] = {'status': 'poor', 'value': lcp, 'threshold': '≤2.5s'}
            assessment['overall_status'] = 'poor'

        # CLS assessment
        cls = lighthouse_analysis.get('cumulative_layout_shift', {}).get('value', 0)
        if cls <= 0.1:
            assessment['metrics']['cls'] = {'status': 'good', 'value': cls, 'threshold': '≤0.1'}
        elif cls <= 0.25:
            assessment['metrics']['cls'] = {'status': 'needs_improvement', 'value': cls, 'threshold': '≤0.1'}
            if assessment['overall_status'] == 'good':
                assessment['overall_status'] = 'needs_improvement'
        else:
            assessment['metrics']['cls'] = {'status': 'poor', 'value': cls, 'threshold': '≤0.1'}
            assessment['overall_status'] = 'poor'

        return assessment

    def _create_performance_optimization_plan(self, lighthouse_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_optimization_priorities(self, summary_data: Dict[str, Any], recommendations: List) -> Dict[str, Any]:
    ...
    def _estimate_optimization_impact(self, comprehensive_recommendations: Dict[str, Any]) -> Dict[str, Any]:
        """Estimate the impact of implementing all recommendations."""

        current_score = comprehensive_recommendations.get('seo_score', {}).get('overall_score', 0)

        # Estimate potential improvement based on recommendations
        critical_fixes = comprehensive_recommendations.get('summary', {}).get('critical_issues', 0)
        high_priority_fixes = comprehensive_recommendations.get('summary', {}).get('high_priority', 0)
    ...
    def _get_score_improvement_recommendations(self, factors: Dict[str, Any]) -> List[str]:
        ...
        for factor, data in factors.items():
            ...
            if impact > 10:
                if factor == 'technical_issues':
                    recommendations.append("Fix critical technical issues to improve crawlability")
                elif factor == 'duplicate_content':
                    recommendations.append("Eliminate duplicate content to avoid keyword cannibalization")
                elif factor == 'broken_pages':
                    recommendations.append("Fix broken pages and implement proper redirects")
...
class OnPageAnalyzer(EnhancedOnPageAnalyzer):
    """Backward compatible OnPageAnalyzer with enhanced features."""

    def __init__(self, client: DataForSEOClient):
        # Initialize with rich reporting disabled by default for backward compatibility
        super().__init__(client, use_rich_reporting=False)