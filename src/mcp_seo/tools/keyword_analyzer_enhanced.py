...
from mcp_seo.models.seo_models import (KeywordAnalysisRequest, KeywordData,
                                       SERPAnalysisRequest)
from mcp_seo.reporting import SEOReporter


class EnhancedKeywordAnalyzer:
    """Enhanced keyword research and analysis tool with advanced reporting and recommendations."""
        ...
        try:
            ...
            for kw in keywords_data:
                keyword_performance_data[kw['keyword']] = {
                    ...
                    'cpc': kw.get('cpc', 0),
                    'gitalchemy_position': None  # Not available in this context
                }

            recommendations = self.recommendation_engine.analyze_keyword_performance(keyword_performance_data)
            result['seo_recommendations'] = [rec.__dict__ for rec in recommendations]

            # Generate keyword targeting strategy
            result['keyword_targeting_strategy'] = self._generate_keyword_targeting_strategy(keywords_data, suggestions)

            # Add formatted report
            result['formatted_report'] = self.reporter.generate_keyword_analysis_report(result)
    ...
    def get_keyword_suggestions(self, seed_keyword: str, location: str = "usa",
        ...
        try:
            ...
            result['suggestion_categories'] = self._categorize_keyword_suggestions(suggestions)
    ...
    def analyze_serp_for_keyword(self, request: SERPAnalysisRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if organic_results:
                content_suggestions = self._generate_content_suggestions_from_serp(organic_results, request.keyword)
    ...
    def get_keyword_difficulty(self, keywords: List[str], location: str = "usa",
        ...
        try:
            ...
            if difficulty_scores:
                targeting_recommendations = self._generate_keyword_targeting_recommendations(difficulty_scores)
    ...
    def _generate_keyword_targeting_strategy(self, keywords_data: List[Dict[str, Any]], suggestions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            'strategy_overview': {
                ...
                'recommended_focus': 'long_tail' if len(long_tail_opportunities) > 5 else 'quick_wins'
            },
            'keyword_categories': {
                'high_volume_low_competition': {
    ...
    def _categorize_keyword_suggestions(self, suggestions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for suggestion in suggestions:
            ...
            # Question based
            if any(q in keyword for q in ['how', 'what', 'why', 'when', 'where', 'who']):
                categories['question_based'].append(suggestion)

            # Commercial intent
            if any(c in keyword for c in ['buy', 'price', 'cost', 'review', 'best', 'compare']):
                categories['commercial_intent'].append(suggestion)

        return categories

    def _generate_content_suggestions_from_serp(self, organic_results: List[Dict[str, Any]], keyword: str) -> Dict[str, Any]:
        """Generate content optimization suggestions based on SERP analysis."""
        ...
        for result in top_results:
            ...
            for word in words:
                if len(word) > 3:  # Ignore short words
                    content_patterns['common_words'][word] = content_patterns['common_words'].get(word, 0) + 1

        # Identify frequently used terms
        frequent_terms = sorted(content_patterns['common_words'].items(), key=lambda x: x[1], reverse=True)[:10]
        ...
        suggestions = {
            'content_optimization': {
                ...
                'featured_snippet_opportunity': {
                    'available': not any(result.get('is_featured_snippet') for result in organic_results),
                    'strategy': 'Create concise, well-formatted answers to common questions' if not any(result.get('is_featured_snippet') for result in organic_results) else 'Improve existing content to compete for featured snippet'
                }
            },
            'competition_analysis': {
                'average_title_length': sum(len(r.get('title', '')) for r in top_results) / len(top_results) if top_results else 0,
                'domains_to_study': [result.get('domain', '') for result in top_results[:3]],
    ...
    def _generate_keyword_targeting_recommendations(self, difficulty_scores: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Generate keyword targeting recommendations based on difficulty analysis."""

        easy_keywords = [k for k in difficulty_scores if k['difficulty_level'] in ['easy', 'medium']]
        hard_keywords = [k for k in difficulty_scores if k['difficulty_level'] in ['hard', 'very_hard']]
...
# Backward compatibility - inherit from enhanced version
class KeywordAnalyzer(EnhancedKeywordAnalyzer):
    """Backward compatible KeywordAnalyzer with enhanced features."""

    def __init__(self, client: DataForSEOClient):
        # Initialize with rich reporting disabled by default for backward compatibility
        super().__init__(client, use_rich_reporting=False)    """Backward compatible KeywordAnalyzer with enhanced features."""
