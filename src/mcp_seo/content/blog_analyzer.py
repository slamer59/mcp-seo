...
class BlogAnalyzer:
    ...
    def generate_comprehensive_analysis(self, metrics: Optional[Dict[str, Dict]] = None) -> Dict[str, Any]:
        """
        ...
        analysis = {
            ...
            'pillar_pages': self._identify_pillar_pages(),
    ...
    def _generate_summary_statistics(self) -> Dict[str, Any]:
        ...
        # Quality metrics
        readability_scores = [post['quality_metrics']['readability_score']
    ...
    def _calculate_content_coverage(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_keywords_per_post': round(total_keywords / len(self.posts_data), 2)
        }

    def _identify_pillar_pages(self, top_n: int = 10) -> List[Dict[str, Any]]:
        ...
        for slug, metric in self.metrics.items():
            ...
            pillar_candidates.append({
                ...
                'readability_score': post.get('quality_metrics', {}).get('readability_score', 0),
    ...
    def _calculate_pillar_score(self, metric: Dict, post: Dict, total_pages: int) -> float:
        ...
        readability_score = post.get('quality_metrics', {}).get('readability_score', 0) / 100 * 0.05
        keyword_score = min(len(post.get('keywords', [])) / 10, 1.0) * 0.1

        return pagerank_score + authority_score + link_score + word_count_score + readability_score + keyword_score

    def _identify_underperforming_pages(self,
                                      pagerank_threshold: float = 0.01,
                                      in_degree_threshold: int = 2) -> List[Dict[str, Any]]:
    ...
    def _analyze_content_clusters(self, min_cluster_size: int = 2) -> Dict[str, List[Dict]]:
            ...
            for keyword in post.get('keywords', []):
                if len(keyword) > 3:  # Filter short keywords
                    keyword_clusters[keyword.lower()].append({
                        ...
                        'word_count': post.get('word_count', 0),
                        'authority_score': self.metrics.get(slug, {}).get('authority_score', 0)
                    })
    ...
    def _analyze_content_quality(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'improvement_candidates': quality_breakdown['poor'] + quality_breakdown['fair']
        }

    def _calculate_content_quality_score(self, quality_metrics: Dict, post: Dict) -> float:
        ...
        return length_score + readability_score + structure_score + media_score + keyword_score
    ...
    def _calculate_link_opportunity(self, source_keywords: set, target_keywords: set, target_slug: str) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if not self.analysis_cache:
            logger.warning("No cached analysis found for recommendations")
        ...
        if pillar_pages:
            ...
            recommendations.append({
                ...
                'metrics': {
                    ...
                    'avg_pagerank': statistics.mean([p['pagerank'] for p in top_pillars])
                }
            })
        ...
        if underperforming:
            ...
            recommendations.append({
                ...
                'metrics': {
                    ...
                    'avg_potential': statistics.mean([p['potential_score'] for p in high_potential]) if high_potential else 0
                }
            })
        ...
        if content_clusters:
            strong_clusters = {k: v for k, v in content_clusters.items() if v['cluster_strength'] > 0.1}
            recommendations.append({
                ...
                'metrics': {
                    ...
                    'total_pages_in_clusters': sum(v['cluster_size'] for v in strong_clusters.values())
                }
            })
        ...
        if improvement_candidates:
            recommendations.append({
                ...
                'metrics': {
                    ...
                    'avg_quality_score': statistics.mean([p['quality_score'] for p in improvement_candidates]) if improvement_candidates else 0
                }
            })
        ...
        if link_opportunities:
            high_value_opportunities = [opp for opp in link_opportunities if opp['score'] > 5]
            recommendations.append({
                ...
                'metrics': {
                    ...
                    'avg_opportunity_score': statistics.mean([opp['score'] for opp in high_value_opportunities]) if high_value_opportunities else 0
                }
            })
    ...
    def get_page_insights(self, slug: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'linking': {
                ...
                'link_targets': [link['target_slug'] for link in post.get('internal_links', [])]
            },
    ...
    def _calculate_page_seo_score(self, post: Dict, metrics: Dict) -> float:
        ...
        content_score = self._calculate_content_quality_score(post.get('quality_metrics', {}), post) / 100
